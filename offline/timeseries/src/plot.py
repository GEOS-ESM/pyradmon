...
# Filter out matplotlib DEBUGGING messages
import logging
...
def fetch_key_from_subplot_dict(subplot_dict):
    ...
    """
    keys = list(map(str, subplot_dict.keys()))
        ...
        for subplotIndex in range(0, len(plot["plots"])):
            ...
            if isset("data", subplot):
                ...
                if isset("x", subplot["data"]):
                    ...
                    for eleID in range(0, len(subplot["data"]["x"])):
                ...
                if isset("y", subplot["data"]):
                    ...
                    for eleID in range(0, len(subplot["data"]["y"])):
                        # Ensure that it's not already substituted in!
...
def plot(plot_dict, data_dict, metadata_dict, rel_channels_dict, custom_vars = None, make_dirs = False):
    ...
    * POST_PROCESSING_FUNC:
        ...
        currently used data for their axes, respectively. Examples::
          ...
          y: '[(data["bla"][i]+y[i]) for i in range(0, len(y))]'
            Equiv: lambda data,x,y: \\
              [ (data["bla"][i] + y[i]) for i in range(0, len(y)) ]
    ...
    * SUBPLOT_TITLE:
    ...
    for plot_id in plot_dict:
        ...
        #plt.subplots_adjust(hspace = 1.2, left=0.15, top=0.86) # broken
        # plt.subplots_adjust(hspace = 1.5, left=0.15, top=0.88) # original
        plt.subplots_adjust(hspace = 0.8, left=0.15, top=0.88) # original
        ...
        for subplotIndex in range(0, len(plot["plots"])):
                    ...
                    if isset("y", subplot["data"]):
                        ...
                        if isset("post_processing", subplot["data"]):
                            ...
                            if isset("x", subplot["data"]["post_processing"]):
                                ...
                                for eleID_x in range(0, len(subplot["data"]["x"])):
                                    for eleID_y in range(0, len(subplot["data"]["y"])):
            ...
            axe.xaxis.set_major_formatter(mdates.DateFormatter('%d-%m-%Y'))     # "01-03-2020"
