...
class ColumnReadSpaces(ColumnReadBase):
    ...
    def _process_data(self):
        ...
        for X in range(0, BINARIZED_DATA_MAX_WIDTH):
            FOUND_EMPTY = True
            for Y in range(0, BINARIZED_DATA_MAX_HEIGHT):
        ...
        for X in range(0, BINARIZED_DATA_MAX_WIDTH_NO_COMMENTS):
            FOUND_EMPTY = True
            for Y in range(0, BINARIZED_DATA_MAX_HEIGHT_NO_COMMENTS):
        ...
        while i < len(EMPTY_COLUMNS):
            ...
            for j in range(EMPTY_COLUMNS[i - 1], EMPTY_COLUMNS[i]):
        ...
        while i < len(BINARIZED_DATA[1]):
            ...
            if BINARIZED_DATA[1][i] == 1:
                SC_START_POS = i
                for j in range(SC_START_POS, len(BINARIZED_DATA[1])):
        ...
        for SUBCOL in SUBCOLUMNS:
            SUBNAME = ""
            for c in range(SUBCOL[0], SUBCOL[1]+1):
                #if BINARIZED_DATA[0][c] == 1:
