...
def print_table(table_dict, pause = True, table_debug = False):
    ...
    for colIndex in range(0, max_cols):
        max_arr = []
        for row in table_dict:
    ...
    for row in table_dict:
        ...
        line_mask = [ True if (x in col_buffer) and (len(col_buffer[x]) > 0) and (col_buffer[x][0] != "") \
                        else False for x in range(0, len(max_widths)) ]
        #print line_mask
        #raw_input()
        while not done_printing:
            print_str = "| "
            all_len = [ len(col_buffer[x]) if x in col_buffer else 0 for x in range(0, len(max_widths)) ]
            
            if sum(all_len) == 0:
                done_printing = True
                break
            
            for col_i in range(0, len(max_widths)):
                #print col_buffer
                blank = False
                #raw_input()
        ...
        if print_plusline:
            cur_line = "|"
            for field_id in range(0, len(max_widths)):
...
def display(pyradmon_config, plot_dict, pause = True, table_debug = False):
    ...
    for plotID in sorted(plot_dict):
        ...
        for subplotIndex in range(0, len(plot["plots"])):
            # Set our subplot!
            subplotID = list(plot["plots"][subplotIndex].keys())[0]
            ...
            if isset("data", subplot):
                if isset("labels", subplot["data"]):
                    for i in range(0, len(subplot["data"]["y"])):
                        if i == 0:
